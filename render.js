...
(function (global) {
  ...
  /* ------------------------------------------
     PATH RESOLUTION
     The site now has two directory depths:
       /            (index.html + shared assets)
       /pages/*.html (all other content pages)
     BASE is the relative prefix that gets a page
     back up to the project root. Every reference
     to icons/, data/, index.html, and the CV file
     goes through BASE so this file stays identical
     regardless of which depth includes it.
  ------------------------------------------ */
  ...
  // Path for a given language: en -> data/content.json, ar -> data/content.ar.json, etc.
  // All content/skills JSON (en + ar + fr, 6 files total) lives in /data.
  function langFile(baseName, lang) {
    return DATA_DIR + (lang === 'en' ? `${baseName}.json` : `${baseName}.${lang}.json`);
  }
  ...
  async function loadContent() {
    ...
    try {
      ...
      if (lang !== 'en') {
        console.warn(`[render.js] Could not load ${langFile('content', lang)} (${err.message}). Falling back to data/content.json.`);
        try {
          const r2 = await fetch(DATA_DIR + 'content.json', { cache: 'no-store' });
          if (!r2.ok) throw new Error(`HTTP ${r2.status}`);
          _content = await r2.json();
        } catch (err2) {
          console.warn(`[render.js] Could not load data/content.json either (${err2.message}). Using built-in fallback data.`);
          _content = _CONTENT_DATA;
        }
      } else {
        console.warn(`[render.js] Could not load data/content.json (${err.message}). Using built-in fallback data.`);
        _content = _CONTENT_DATA;
      }
    }
    return _content;
  ...
  async function loadSkills() {
    ...
    try {
      ...
      if (lang !== 'en') {
        console.warn(`[render.js] Could not load ${langFile('skills', lang)} (${err.message}). Falling back to data/skills.json.`);
        try {
          const r2 = await fetch(DATA_DIR + 'skills.json', { cache: 'no-store' });
          if (!r2.ok) throw new Error(`HTTP ${r2.status}`);
          _skills = await r2.json();
        } catch (err2) {
          console.warn(`[render.js] Could not load data/skills.json either (${err2.message}). Using built-in fallback data.`);
          _skills = _SKILLS_DATA;
        }
      } else {
        console.warn(`[render.js] Could not load data/skills.json (${err.message}). Using built-in fallback data.`);
        _skills = _SKILLS_DATA;
      }
    }
    return _skills;
...
'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
  ...
  function renderHeader(meta, { showHomeBtn = true } = {}) {
    ...
    if (!document.getElementById('header-overlay-btns')) {
      ...
      overlay.innerHTML = `
        ${showHomeBtn ? `
          <a href="${BASE}index.html" class="home-button float-btn" aria-label="Go to home">
            <img src="${ICONS_DIR}home.png" alt="Home" class="icon">
          </a>` : ''}
        ...
        <button id="theme-toggle" class="theme-toggle float-btn" aria-label="Toggle theme">
          <img src="${ICONS_DIR}moon.png" alt="Toggle theme" class="icon">
        </button>
      `;
      ...
      <div class="header-bar">
        ...
        <div class="header-text">
          ...
          <p>
            ...
            <a href="${meta.boldpro}"  target="_blank" rel="noopener"></a>
          </p>
        </div>
        <div class="header-photo-wrap">
          <img src="${ICONS_DIR}face.jpeg" alt="Profile photo" class="profile-pic">
        </div>
      </div>
    `;
  ...
  // NOTE: these are plain filenames, not paths. All pages that call renderNav
  // live together as siblings inside /pages, so no './' or '../' prefix is
  // needed here — only index.html (which never calls renderNav) is elsewhere.
  const NAV_KEYS = [
  ...
  function renderBreadcrumb(activePage) {
    ...
    bc.innerHTML = `<a href="${BASE}index.html">Home</a> <span class="bc-sep">›</span> <span class="bc-current">${NAV_LABELS_EN[link.key]}</span>`;
  ...
  function renderFloatingButtons() {
    ...
    el.innerHTML = `
      ...
      <button class="float-btn to-contact" type="button" aria-label="Go to contact">
        <img src="${ICONS_DIR}contact.png" alt="Contact" class="icon">
      </button>
    `;
  }
  ...
  function renderContact(meta, ui) {
    ...
    // Always resolve to the project ROOT, regardless of how deep the current
    // page is nested (index.html vs pages/*.html), since the CV file lives
    // at the root next to index.html.
    const siteURL  = new URL(BASE, window.location.href).href;
      ...
      <div class="contact-row">
        <div class="contact-info">
          ...
          <p>${t('phone', 'Phone:')} <bdi dir="ltr"><a href="${meta.phone1.wa}">${meta.phone1.display}</a></bdi>
            <img src="${ICONS_DIR}wts.png" alt="WhatsApp" class="icon"></p>
        ...
        <div class="contact-download">
          <div class="share-btn-wrap">
            <button class="cv-btn" id="share-resume-btn" type="button">
              <img src="${ICONS_DIR}dwn.png" alt="" class="icon"> ${t('resumeBtn', 'Resume &amp; Portfolio')}
            </button>
            <div class="share-panel" id="share-panel" hidden>
              <a href="${BASE}${meta.cvFile}" class="share-option" download>
              ...
              <a href="https://wa.me/?text=${waText}" class="share-option" target="_blank" rel="noopener">
                <span class="share-opt-icon"><img src="${ICONS_DIR}wts.png" alt="" class="icon share-opt-icon-img"></span> ${t('shareWhatsapp', 'Share via WhatsApp')}
              </a>
        ...
        <div class="contact-qr">
          <img src="${ICONS_DIR}QR.png" alt="QR code" class="qr-code">
...
const counterImg = location.protocol !== 'file:'
  ...
  function skillChips(skillArray) {
    ...
    return skillArray.map(s => {
      ...
      return `<a href="skills.html#${slug}" class="skill-tag-mini">${s.replace(/-/g, ' ')}</a>`;
    }).join('');
  }
